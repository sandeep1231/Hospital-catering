...
async function writeAudit(entity: string, entityId: any, action: string, user: any, details?: any) {
  try { await AuditLog.create({ entity, entityId, action, userId: user?.id || user?._id || null, hospitalId: user?.hospitalId || null, vendorId: user?.vendorId || null, details }); } catch (e) { console.error('audit error', e); }
}
...
router.put('/:id/kitchen-status', auth, requireRole('kitchen', 'vendor', 'admin'), requireWriteAccess(), async (req: Request, res: Response) => {
  const user = (req as any).user;
...
router.put('/:id/deliver', auth, requireRole('delivery', 'admin'), requireWriteAccess(), async (req: Request, res: Response) => {
  const user = (req as any).user;
...
router.put('/:id/status', auth, requireRole('admin'), requireWriteAccess(), async (req: Request, res: Response) => {
  const user = (req as any).user;
...
router.post('/bulk-deliver', auth, requireRole('admin'), requireWriteAccess(), async (req: Request, res: Response) => {
  const user = (req as any).user;
